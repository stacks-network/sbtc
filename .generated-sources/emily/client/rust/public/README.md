# Rust API client for emily-client
...
Put the package under your project folder in a directory named `emily-client` and add the following to `Cargo.toml` under `[dependencies]`:

```
emily-client = { path = "./emily-client" }
```
