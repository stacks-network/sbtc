    ...
    N: network::MessageTransfer,
{
    ...
    /// Polls for [`Msg`]'s from the network and our local tx signer, combining
    /// them into a single vec.
    async fn poll_for_tx_signer_messages(&mut self) -> Result<Vec<Msg>, Error> {
        let mut signal_rx = self.context.get_signal_receiver();
    ...
    async fn relay_messages_to_wsts_state_machine_until_signature_created(
        ...
        loop {
            // Poll for messages from the network and our own signer.
            let signer_messages = self.poll_for_tx_signer_messages().await?;
