# Rust API client for signer-emily-client
...
Put the package under your project folder in a directory named `signer-emily-client` and add the following to `Cargo.toml` under `[dependencies]`:

```
signer-emily-client = { path = "./signer-emily-client" }
```
