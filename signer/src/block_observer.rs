        ...
        loop {
            ...
            match poll.await {
                Ok(Some(Ok(block_hash))) => {
                    ...
                    let block_headers = match self.next_blocks_to_process(block_hash).await {
                        Ok(blocks) => blocks,
                        Err(error) => {
...
impl<C: Context, B> BlockObserver<C, B> {
    ...
    /// equal to the sbtc_start_height.
    ...
    async fn next_blocks_to_process(
        ...
        let stop_height = self.context.state().get_sbtc_start_height();
    ...
    async fn process_bitcoin_block(&self, block_header: BitcoinBlockHeader) -> Result<(), Error> {
        ...
        self.write_bitcoin_block(&block_header).await?;
    ...
    async fn write_bitcoin_block(&self, block_header: &BitcoinBlockHeader) -> Result<(), Error> {
        let block = self
            .context
            .get_bitcoin_client()
