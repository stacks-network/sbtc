...
async fn can_store_and_get_latest_sweep_transaction_package() {
    ...
    let mut expected1 = setup
        .store_sweep_transaction_packages(&db)
        .await
        .pop()
        .unwrap();
    // This is the id that will be assigned by the database for the transaction
    // generated by the 1st `TestSweepSetup`.
    expected1.transactions[0].id = Some(1);
    // `store_sweep_transaction_packages()` below also marks each transaction
    // as having been broadcast.
    expected1.transactions[0].is_broadcast = true;
    ...
    for block in faucet.generate_blocks(10) {
        sqlx::query(
            "INSERT INTO sweep_packages (created_at_block_hash, market_fee_rate) VALUES ($1, $2);",
        )
        .bind(BitcoinBlockHash::from(block))
        .bind(0.0)
        .execute(db.pool())
        .await
        .expect("failed to insert dummy sweep package");
        crate::setup::backfill_bitcoin_blocks(&db, rpc, &block).await;
    }

    // Store outstanding sweep transaction packages in the database and get the
    // expected sweep package for asserts later on.
    let mut expected2 = setup
        .store_sweep_transaction_packages(&db)
        .await
        .pop()
        .unwrap();
    // This is the id that will be assigned by the database for the transaction
    // generated by the 2nd `TestSweepSetup`.
    expected2.transactions[0].id = Some(2);
    // `store_sweep_transaction_packages()` below also marks each transaction
    // as having been broadcast.
    expected2.transactions[0].is_broadcast = true;
    ...
    // ** TEST 1 **
    // Assert that if we request the latest sweep package for the first
    // blockchain tip, we get the first sweep package.
    let package1 = db
        .get_latest_sweep_transaction_package(&first_sweep_block_hash.into(), 50)
        .await
        .expect("failed to get latest sweep transaction package (on fork)");

    let Some(package1) = package1 else {
        panic!("expected to find a sweep transaction package");
    };

    assert_eq!(package1, expected1);

    // ** TEST 2 **
    // Assert that if we request the latest sweep package for the second
    // blockchain tip, we get the second sweep package.
    //
    // This test also verifies that the sweep package pre-insert is identical to
    // the sweep package when retrieved from the database.
    let package2 = db
        .get_latest_sweep_transaction_package(&setup.sweep_block_hash.into(), 50)
        .await
        .expect("failed to get latest sweep transaction package");

    let Some(package2) = package2 else {
        panic!("expected to find a sweep transaction package");
    };

    assert_eq!(package2, expected2);
}
