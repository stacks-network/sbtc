...
async fn adding_and_then_updating_single_accout_limit_works() {
    ...
    .iter()
    .map(|(account_name, limits)| {
        (account_name.to_string(), limits.clone())
    }).collect();
    ...
    for (account_name, _) in expected_account_caps.clone() {
        individually_retrieved_account_caps.insert(
            account_name.clone(),
            apis::limits_api::get_limits_for_account(
                &configuration,
                &account_name,
            )
            .await
...
async fn test_updating_account_limits_via_global_limit_works() {
    ...
    let global_limits_returned_on_set = apis::limits_api::set_limits(&configuration, global_limits_to_set.clone())
        .await
    ...
    // Assert.
    // -------
    assert_eq!(individually_retrieved_account_caps, expected_global_account_limits);
