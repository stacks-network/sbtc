# Rust API client for admin-emily-client
...
Put the package under your project folder in a directory named `admin-emily-client` and add the following to `Cargo.toml` under `[dependencies]`:

```
admin-emily-client = { path = "./admin-emily-client" }
```
