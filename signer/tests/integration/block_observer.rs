...
async fn load_latest_deposit_requests_persists_requests_from_past(blocks_ago: u64) {
    ...
    let setup0 = TestSweepSetup::new_setup(&bitcoin_client, faucet, 100_000, &mut rng);
    let setup1 = TestSweepSetup::new_setup(&bitcoin_client, faucet, 200_000, &mut rng);
...
async fn next_headers_to_process_ignores_known_headers() {
    ...
    let context = TestContext::builder()
        .with_storage(db.clone())
        .with_bitcoin_client_from_url(&bitcoind.get_rpc_endpoint())
        .with_mocked_emily_client()
        .with_mocked_stacks_client()
        .build();
    ...
    let chain_tip_block_hash = bitcoin_client.get_best_block_hash().unwrap();
    let headers = block_observer
