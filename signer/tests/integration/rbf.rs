...
pub async fn transaction_with_rbf(
    ...
    let deposits: Vec<DepositRequest> =
        std::iter::repeat_with(|| generate_depositor(&client, faucet, &signer))
    ...
    let fees = {
        ...
        // ** Step 2 **
        // Create an RBF transaction that will fail.
        //
        ...
        let one_response: Result<Vec<Txid>, BtcRpcError> = transactions
            ...
            .map(|unsigned| {
                signer::testing::set_witness_data(unsigned, signer.keypair);
                client.send_raw_transaction(&unsigned.tx)
            })
            .collect();
        match one_response {
    ...
    let fee_map: std::collections::HashMap<ScriptPubKey, u64> = transactions
        ...
        .flat_map(|utx| {
            utx.requests
                ...
                // We only care about the outputs that correspond to
    ...
    for (index, (req, recipient)) in iter {
        let balance = recipient.get_balance(&client);
