...
emily-integration-test: devenv $(EMILY_LAMBDA_BINARY) $(EMILY_CDK_TEMPLATE) $(EMILY_DOCKER_COMPOSE)
	...
	rm -f devenv/dynamodb/data/shared-local-instance.db
	@echo "Starting integration test environment."
	CONTAINER_HOST=$(_CONTAINER_HOST) docker compose \
		--profile test \
		--file $(EMILY_DOCKER_COMPOSE) up \
		--remove-orphans
...
devenv: $(wildcard $(subst dir, devenv, $(TWO_DIRS_DEEP)))
	docker compose --file docker-compose.emily.yml build
	@touch devenv

emily-lambda: $(EMILY_LAMBDA_BINARY)

emily-local-test: build
	cargo test --package emily-lambda -- --nocapture

emily-curl-test: $(EMILY_LAMBDA_BINARY)
	./devenv/service-test/curl-test.sh localhost 3000
...
$(EMILY_LAMBDA_BINARY): $(AUTOGENERATED_EMILY_CLIENT) $(wildcard $(subst dir, emily/lambda, $(THREE_DIRS_DEEP)))
