...
class NewBlockEventModel(BaseModel):
    block_height: int
...
api_client = emily_client.ApiClient(
    configuration=conf
)
...
@app.post("/new_block")
async def handle_new_block(request: Request) -> dict:
    try:
        body = await request.json()
        event = NewBlockEventModel(**body)  # Validate using Pydantic
    except ValidationError as e:
        logger.error("Validation error: %s", e.errors())
        raise HTTPException(status_code=400, detail={"error": "Invalid data", "messages": e.errors()})
    ...
    try:
        ...
        logger.error("Failed to send chainstate %s to %s: %s", chainstate, emily_endpoint, e)
