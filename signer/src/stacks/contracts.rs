...
impl AsContractCall for CompleteDepositV1 {
    ...
    /// 7. That the fee matches the expected assessed fee for the output.
    /// 8. That the fee is less than the desired max-fee.
...
impl CompleteDepositV1 {
    ...
    /// 6. That the amount to mint does not exceed the deposit amount.
    /// 7. That the fee matches the expected assessed fee for the output.
    /// 8. That the max-fee is less than the desired max-fee.
    ///
        ...
        S: DbRead + Send + Sync,
    {
        ...
        // 8. Check that the fee is less than the desired max-fee.
        //
        ...
        C: Context + Send + Sync,
    {
        ...
        let script_pub_key = sweep_tx
            .vin
            .first()
            .map(|x| ScriptBuf::from_bytes(x.prevout.script_pub_key.hex.clone()))
        ...
        // the signers.
        if !db.is_signer_script_pub_key(&script_pub_key.into()).await? {
