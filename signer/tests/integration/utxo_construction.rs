...
async fn helper_struct_methods_work() {
    ...
    // their address.
    let balance = signer.get_balance(&bitcoin_client);
    ...
    let balance = withdrawal_recipient.get_balance(&bitcoin_client);
    assert_eq!(balance.to_sat(), 0);

    // Now we check that get_utxos do what we want
    let mut utxos = signer.get_utxos(&bitcoin_client, None);
...
async fn deposits_add_to_controlled_amounts() {
    ...
    assert_eq!(depositor.get_balance(&bitcoin_client).to_sat(), 50_000_000);

    // Now lets make a deposit transaction and submit it
    let depositor_utxo = depositor.get_utxos(&bitcoin_client, None).pop().unwrap();
    ...
    let depositor_balance = depositor.get_balance(&bitcoin_client);
...
async fn withdrawals_reduce_to_signers_amounts() {
    ...
    assert_eq!(recipient.get_balance(&bitcoin_client).to_sat(), 0);
    ...
    let signers_balance = signer.get_balance(&bitcoin_client).to_sat();
    ...
    let recipient_balance = recipient.get_balance(&bitcoin_client).to_sat();
    ...
    let another_recipient_balance = another_recipient.get_balance(&bitcoin_client).to_sat();
    assert_eq!(another_recipient_balance, 0);

    // Get the UTXO that the signer sent to the withdrawing user.
    let withdrawal_utxo = recipient.get_utxos(&bitcoin_client, None).pop().unwrap();
