...
export class EmilyStack extends cdk.Stack {
    ...
    constructor(scope: Construct, id: string, props: EmilyStackProps) {
        ...
        const heightsMappingTableId: string = 'heightsMappingTable';
        const heightsMappingTableName: string = EmilyStackUtils.getResourceName(heightsMappingTableId, props);
        const heightsMappingTable: dynamodb.Table = this.createOrUpdateHeightsMappingTable(
            heightsMappingTableId,
            heightsMappingTableName,
            persistentResourceRemovalPolicy,
            pointInTimeRecovery,
        );

        const bitcoinChainstateTableId: string = 'bitcoinChainstateTable';
        const bitcoinChainstateTableName: string = EmilyStackUtils.getResourceName(bitcoinChainstateTableId, props);
        const bitcoinChainstateTable: dynamodb.Table = this.createOrUpdateBitcoinChainstateTable(
            bitcoinChainstateTableId,
            bitcoinChainstateTableName,
            persistentResourceRemovalPolicy,
            pointInTimeRecovery,
        );
    ...
    /**
     * Creates or updates a DynamoDB table for heights mapping.
     * @param {string} tableId The id of the table AWS resource.
     * @param {string} tableName The name of the DynamoDB table.
     * @returns {dynamodb.Table} The created or updated DynamoDB table.
     * @post A DynamoDB table is returned without additional configuration.
     */
        createOrUpdateHeightsMappingTable(
            tableId: string,
            tableName: string,
            removalPolicy: cdk.RemovalPolicy,
            pointInTimeRecovery: undefined | boolean,
     ...
     * @post A DynamoDB table is returned without additional configuration.
     */
    createOrUpdateBitcoinChainstateTable(
        tableId: string,
        tableName: string,
        removalPolicy: cdk.RemovalPolicy,
        pointInTimeRecovery: undefined | boolean,
    ...
    createOrUpdateOperationLambda(
        ...
        heightsMappingTableName: string,
        bitcoinChainstateTableName: string,
        removalPolicy: cdk.RemovalPolicy,
        ...
        const operationLambda: lambda.Function = new lambda.Function(this, operationLambdaId, {
            ...
            environment: {
                ...
                HEIGHTS_MAPPING_TABLE_NAME: heightsMappingTableName,
                BITCOIN_CHAINSTATE_TABLE_NAME: bitcoinChainstateTableName,
