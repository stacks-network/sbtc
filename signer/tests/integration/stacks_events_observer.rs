...
fn make_webhook_payload(
    ...
    "cycle_number": null,
    "events": [],
    "index_block_hash": "{block_hash_hex}",
    ...
    "transactions": [
        ...
        block_height = block_height,
        parent_index_block_hash_hex = parent_index_block_hash_hex,
        burn_block_hash_hex = burn_block_hash_hex,
        burn_block_height = burn_block_height,
        parent_burn_block_hash_hex = parent_burn_block_hash_hex,
        parent_burn_block_height = burn_block_height - 1
    )
}

fn generate_mock_events(
    rng: &mut impl rand::RngCore,
    num_bitcoin_blocks: usize,
    num_stacks_blocks_per_bitcoin_block: std::ops::Range<usize>,
) -> Vec<(NewBlockEvent, String)> {
    ...
    stx_blocks
        ...
        .map(|(index, (id, block, btc_hash))| {
            let event_str: String = make_webhook_payload(
                &id.to_hex(),
            ...
            (
                serde_json::from_str::<NewBlockEvent>(&event_str).unwrap(),
                event_str,
            )
        })
        .collect()
...
async fn test_new_blocks_sends_set_chainstate_to_emily_happy_path() {
    ...
    let events = generate_mock_events(&mut rng, 5, 1..3);
    ...
    assert!(wipe_databases(&emily_context).await.is_ok());
    ...
    for (event, event_str) in events.iter() {
        let status_code = new_block_handler(state.clone(), event_str.to_owned()).await;
        assert_eq!(status_code, StatusCode::OK);

        let resp = get_chain_tip(&emily_context).await.unwrap();
        assert_eq!(resp.stacks_block_height, event.block_height);
        assert_eq!(resp.stacks_block_hash, event.index_block_hash.to_hex());
    }
}
...
async fn test_new_blocks_sends_set_chainstate_to_emily_repeated_messages() {
    let mut rng = rand::rngs::StdRng::seed_from_u64(46);
    let events = generate_mock_events(&mut rng, 1, 2..3);
    let context = test_context().await;
    let state = State(ApiState { ctx: context.clone() });
    let emily_context = state.ctx.emily_client.config();
    ...
    let (event, event_str) = events_iter.next().unwrap(); // pick an arbitrary event (5th block)
    for _ in 0..5 {
        let status_code = new_block_handler(state.clone(), event_str.to_owned()).await;
        assert_eq!(status_code, StatusCode::OK);
    ...
    let (event, event_str) = events_iter.next().unwrap(); // pick an arbitrary event (6th block)
    let status_code = new_block_handler(state.clone(), event_str.to_owned()).await;
    assert_eq!(status_code, StatusCode::OK);
...
async fn test_new_blocks_sends_set_chainstate_to_emily_skip_messages() {
    let mut rng = rand::rngs::StdRng::seed_from_u64(46);
    let events = generate_mock_events(&mut rng, 2, 2..3);
    let context = test_context().await;
    let state = State(ApiState { ctx: context.clone() });
    let emily_context = state.ctx.emily_client.config();
    ...
    let mut events_iter = events.into_iter();
    // Send first event
    let (event_1, event_str_1) = events_iter.next().unwrap();
    let status_code = new_block_handler(state.clone(), event_str_1.to_owned()).await;
    assert_eq!(status_code, StatusCode::OK);

    let resp = get_chain_tip(&emily_context).await.unwrap();
    assert_eq!(resp.stacks_block_height, event_1.block_height);
    assert_eq!(resp.stacks_block_hash, event_1.index_block_hash.to_hex());

    // Skip sending the 2nd event
    let (skipped_event, skipped_event_str) = events_iter.next().unwrap();

    // Send 3th block event, should be ignored
    let (event_3, event_str_3) = events_iter.next().unwrap();
    let status_code = new_block_handler(state.clone(), event_str_3.to_owned()).await;
    assert_eq!(status_code, StatusCode::OK);
    ...
    let status_code = new_block_handler(state.clone(), skipped_event_str.to_owned()).await;
    assert_eq!(status_code, StatusCode::OK);
    ...
    assert_eq!(
        ...
        skipped_event.index_block_hash.to_hex()
    );
    ...
    assert_eq!(resp.stacks_block_hash, event_3.index_block_hash.to_hex());
}
...
async fn test_new_blocks_sends_set_chainstate_to_emily_starts_reorg() {
    let mut rng = rand::rngs::StdRng::seed_from_u64(46);
    let events = generate_mock_events(&mut rng, 5, 2..3);
    let context = test_context().await;
    let state = State(ApiState { ctx: context.clone() });
    let emily_context = state.ctx.emily_client.config();
    ...
    for (event, event_str) in events.iter().take(3) {
        let status_code = new_block_handler(state.clone(), event_str.to_owned()).await;
        assert_eq!(status_code, StatusCode::OK);
    ...
    let (mut fork_event, _) = events_iter.next().unwrap();
    events_iter.next();
    let (event_4, _) = events_iter.next().unwrap();

    fork_event.index_block_hash = event_4.index_block_hash.clone();
    let fork_event_str = make_webhook_payload(
        &fork_event.index_block_hash.to_hex(),
        fork_event.block_height,
        &fork_event.parent_index_block_hash.to_hex(),
        &fork_event.burn_block_hash.to_hex(),
        fork_event.burn_block_height as u64,
        &fork_event.parent_burn_block_hash.to_hex(),
    );

    // Send the forked block
    let status_code = new_block_handler(state.clone(), fork_event_str.to_owned()).await;
    assert_eq!(status_code, StatusCode::OK);

    // Check that the chain tip is updated with the new fork's hash
    let resp = get_chain_tip(&emily_context).await.unwrap();
    assert_eq!(resp.stacks_block_height, fork_event.block_height);
    assert_eq!(resp.stacks_block_hash, fork_event.index_block_hash.to_hex());
}
